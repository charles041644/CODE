...
def nms(type,tensor_window_size,nms_window_size, threshold, image_path, skip, R, gray_shape):
    image = cv2.imread(image_path)  
    radius = 0 
    ...
    for i in range(radius - 1, gray_shape[1] - radius):
        j = radius - 1
        while ((j < (gray_shape[0] - radius)) and (skip[i, j] or (R[i, j - 1] > R[i, j]))):
            j = j + 1
        while (j < (gray_shape[0] - radius)):
            while ((j < (gray_shape[0] - radius)) and (skip[i, j] or (R[i, j + 1] > R[i, j]))):
                j = j + 1
            if (j < (gray_shape[0] - radius)):
                p1 = j + 2
                ...
                j = p1
    cv2.imwrite('b_' + type + '_window_size_' + str(tensor_window_size) + 'threshold_' + str(threshold) + '.jpg',image)

if __name__ == '__main__':
    # image = cv2.imread("original.jpg")
    image ="NTHU_CV_HW1/1a_notredame.jpg"
    image_shape = np.shape(image)
    center = (image_shape[0] / 2, image_shape[1] / 2)
    threshold = 4

    rotate = cv2.getRotationMatrix2D(center, 30, 1)
    image_rotate = cv2.warpAffine(image, rotate, (image_shape[1], image_shape[0]))
    ...
    nms(type="rotate",tensor_window_size="3", nms_window_size=3, threshold=threshold, image_path="a_1_G_blur_k_size_10.jpg", skip=skip, R=R,
        gray_shape=gray_shape)

    scale = cv2.getRotationMatrix2D(center, 0, 0.5)
    image_scale = cv2.warpAffine(image, scale, (image_shape[1], image_shape[0])) 
    cv2.imwrite('image_scale.jpg', image_scale)
    image_path = 'image_scale.jpg'

    gaussian_smooth(image_path, kernel_size=10, sigma=5)
    skip, R, gray_shape = structure_tensor_get_R(window_size=3, image_path="a_1_G_blur_k_size_10.jpg",
                                                 threshold=threshold)
    nms(type="scale",tensor_window_size="3", nms_window_size=3, threshold=threshold, image_path="a_1_G_blur_k_size_10.jpg",
        skip=skip, R=R,gray_shape=gray_shape)